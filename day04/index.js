  ...
  lines.forEach(text => {
    if (text.includes(",")) {
      result.draws = text
        ...
        .map(number => (number === 0 ? 100 : number));
      return;
    }
    if (!text.trim()) {
      currentBoard = [];
...
function playBingoAndGetLosingBoardAndNumber({ draws, boards }) {
  for (let i = 0; i < draws.length; i++) {
    const currentDraw = draws[i];
    const allIndexesOfStillPlayingBoards = boards
      .map((item, index) => (item == null ? null : index))
    ...
    const allIndexesOfJustWinningBoards = boards
      .map(item => (item == null ? false : checkIfBoardHasWon(item)))
      .map((hasWon, index) => (hasWon ? index : null))
      .filter(index => index != null);
    if (allIndexesOfJustWinningBoards.length === allIndexesOfStillPlayingBoards.length) {
      return calculateScore({
        winningBoard: boards[allIndexesOfJustWinningBoards[0]],
        currentDraw,
      });
    }
